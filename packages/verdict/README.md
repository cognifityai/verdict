...
```bash
pip install "cognifity-verdict[telemetry]==0.1.0a16"  # extra is for OTLP protobuf
...
[`0.1.0a16 POC release profile`](https://github.com/cognifityai/verdict/blob/v0.1.0a16/docs/POC_RELEASE_PROFILE.md).
...
The `0.1.0a16` POC profile uses `buffered_writes=False`. Buffered mode requires
...
```bash
python -m pip install "cognifity-verdict[dashboard]==0.1.0a16"
verdict-dashboard --storage sqlite:///./verdict.db
```
...
For published release `0.1.0a16`, the bounded POC entry points include Anthropic
