...
def _persist_span(sp: Span) -> None:
    ...
    try:
        ...
        if sp.trace_link_state in {
            TraceLinkState.PENDING_PROVIDER,
            TraceLinkState.INHERITED_PENDING_PROVIDER,
        }:
            # The provider trace write has not been acknowledged yet. Its
            # success/failure callback persists this span with the final state.
            return

        linked_trace_id = sp.trace_id
        attributes = dict(sp.attributes)
        if linked_trace_id is not None and sp.trace_link_state in {
            TraceLinkState.EXPLICIT,
            TraceLinkState.INHERITED_EXPLICIT,
        }:
            try:
