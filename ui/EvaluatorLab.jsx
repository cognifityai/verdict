...
export function EvaluatorLab({ configUrl, onOpenEvaluated }) {
  ...
  return <fieldset disabled={busy} className="max-w-5xl space-y-4" style={{ border: 0, margin: 0, padding: 0 }}>
    ...
    {preview && <section className="border p-5" style={{ borderColor: C.border, background: C.panel }}>
      ...
      <label className="flex gap-2 mt-4 text-sm"><input disabled={!previewCurrent} type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} />I approve sending the selected redacted prompt/response evidence to {destinationLabel} and accept the displayed call count and maximum estimated cost.</label>
      <button disabled={!previewCurrent || !confirmed || !providerState?.configured || preview.plannedCalls === 0} onClick={async () => { const data = await post("/api/evaluators/run", { ...payload(), planFingerprint: preview.planFingerprint, plannedTraces: preview.plannedTraces, confirmExternalEgress: true }, "evaluation"); if (data) setResult(data); }} className="mt-4 px-4 py-2 text-sm" style={{ background: C.green, color: "#0b0e0d" }}>{evaluationRunning ? <span className="flex items-center gap-2"><LoaderCircle aria-hidden="true" size={15} className="animate-spin" />Running {preview.plannedCalls.toLocaleString()} judge calls…</span> : `Run ${preview.plannedCalls} judge calls`}</button>
