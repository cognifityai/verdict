  ...
  return <div className="max-w-5xl space-y-4">
    <section className="border p-5" style={{ borderColor: C.border, background: C.panel }}>
      ...
      <div className="grid sm:grid-cols-2 gap-4 mt-5">
        <label className="text-sm">Provider<select value={provider} onChange={(event) => setProvider(event.target.value)} className="block w-full border p-2 mt-1 bg-transparent">{["anthropic", "openai", "google"].map((name) => <option key={name}>{name === "openai" ? "openai / compatible endpoint" : name}</option>)}</select></label>
