...
export function EvaluatorLab({ configUrl, onOpenEvaluated }) {
  ...
  const [evaluationStartedAt, setEvaluationStartedAt] = useState(null);
  ...
  useEffect(() => {
    if (busyAction !== "evaluation" || evaluationStartedAt == null) return undefined;
    const update = () => setEvaluationElapsedSeconds(
      Math.max(0, Math.floor((Date.now() - evaluationStartedAt) / 1000)),
    );
    update();
  ...
  async function post(path, body, action) {
    ...
    finally { requestInFlight.current = false; setBusyAction(null); }
  }
  ...
  return <fieldset disabled={busy} className="max-w-5xl space-y-4" style={{ border: 0, margin: 0, padding: 0 }}>
    <section className="border p-5" style={{ borderColor: C.border, background: C.panel }}>
      ...
      <p className="text-sm mt-2" style={{ color: C.sub }}>Choose provider Trace for individual model calls or Agent Turn for a completed, untruncated request and final output. Turn evaluation does not include tool/context provenance. Preview does not send data externally.</p>
    ...
    {preview && <section className="border p-5" style={{ borderColor: C.border, background: C.panel }}>
      ...
      <label className="flex gap-2 mt-4 text-sm"><input disabled={!previewCurrent} type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} />I approve sending the selected redacted {unit === "agent_turn" ? "Turn request/final output" : "Trace prompt/response"} evidence to {destinationLabel} and accept the displayed call count and maximum estimated cost.</label>
    ...
    <section className="border p-5" style={{ borderColor: C.border, background: C.panel }}>
      ...
      <button disabled={!labelSetPath || !token} onClick={async () => { const data = await post("/api/evaluators/calibration/preview", { ...payload(), labelSetPath }, "calibration-preview"); if (data) setCalibration({ preview: data, result: null, confirmed: false }); }} className="border px-4 py-2 text-sm mt-3">Preview label set</button>
