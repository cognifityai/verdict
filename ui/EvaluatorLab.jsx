...
export function EvaluatorLab({ configUrl, onOpenEvaluated }) {
  ...
  const payload = (cursor = turnCursor) => ({
    ...
    unit, ...(unit === "agent_turn" ? { scanLimit: 100,
      ...(includeToolCounts ? { toolEvidence: "counts_v1" } : {}),
  ...
  return <fieldset disabled={busy} className="max-w-5xl space-y-4" style={{ border: 0, margin: 0, padding: 0 }}>
    ...
    {preview && <section className="border p-5" style={{ borderColor: C.border, background: C.panel }}>
      ...
      {preview.unit === "agent_turn" && <div className="text-xs mt-2" style={{ color: C.sub }}>Tool evidence: {preview.toolEvidence === "counts_v1" ? "recorded counts only; no source verification" : "not included"}.</div>}
