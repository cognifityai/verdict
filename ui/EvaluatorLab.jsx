...
export function EvaluatorLab({ configUrl, onOpenEvaluated }) {
  ...
  return <fieldset disabled={busy} className="max-w-5xl space-y-4" style={{ border: 0, margin: 0, padding: 0 }}>
    ...
    {preview && <section className="border p-5" style={{ borderColor: C.border, background: C.panel }}>
      ...
      <label className="flex gap-2 mt-4 text-sm"><input disabled={!previewCurrent} type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} />I approve sending the selected redacted {unit === "agent_turn" ? "Turn request/final output" : "Trace prompt/response"} evidence to {destinationLabel} for the displayed call count. The cost is an estimate, not a billing cap.</label>
