  ...
  return <div className="max-w-5xl space-y-4">
    <section className="border p-5" style={{ borderColor: C.border, background: C.panel }}>
      ...
      <div className="grid sm:grid-cols-2 gap-4 mt-5">
        ...
        <label className="text-sm"><span className="flex items-center gap-2"><input type="checkbox" checked={judgeAll} onChange={(event) => setJudgeAll(event.target.checked)} />Judge all eligible traces</span><input aria-label="Maximum judge calls" disabled={judgeAll} type="number" min="1" max="10000" value={maxCalls} onChange={(event) => setMaxCalls(Number(event.target.value))} className="block w-full border p-2 mt-2 bg-transparent disabled:opacity-50" /><span className="block text-xs mt-1" style={{ color: C.faint }}>{judgeAll ? "All eligible traces in the bounded 10,000-trace scan." : "Stop after this many eligible, not-yet-evaluated traces."}</span></label>
