...
For a customer proof of concept on `0.1.0a9`, follow the bounded
...
```bash
python -m pip install \
  "cognifity-verdict[dashboard,postgres]==0.1.0a9" \
  "cognifity-verdict-eval==0.1.0a9" \
  "cognifity-verdict-inspect==0.1.0a9"
```
...
controls what fraction of supported calls is retained. The `0.1.0a9` POC
...
- **Published capture coverage in `0.1.0a9`:** the bounded POC profile names
  Anthropic
...
- The `0.1.0a9` POC drift demonstration assumes independently sampled calls.
