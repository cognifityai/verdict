...
1. Install or refresh the `0.1.0a13` skill using a native skill installer or the
...
mkdir /absolute/path/to/cognifity-skills/verdict-instrument-app-0.1.0a13
curl -fsSL https://github.com/cognifityai/verdict/archive/refs/tags/v0.1.0a13.tar.gz \
  | tar -xz \
      -C /absolute/path/to/cognifity-skills/verdict-instrument-app-0.1.0a13 \
      --strip-components=3 \
      verdict-0.1.0a13/skills/verdict-instrument-app
```

This creates
`/absolute/path/to/cognifity-skills/verdict-instrument-app-0.1.0a13/SKILL.md`.
...
```text
Read /absolute/path/to/cognifity-skills/verdict-instrument-app-0.1.0a13/SKILL.md completely
...
synchronized `0.1.0a5`–`0.1.0a12` upgrade, current `0.1.0a13`, mixed-package repair, and
...
schedules through the dashboard, or send outbound alerts that Verdict `0.1.0a13`
does not ship.

## Release-specific boundary

The skill targets public Verdict `0.1.0a13`; its capture-method evidence is the
bounded `0.1.0a13` [POC release profile](POC_RELEASE_PROFILE.md). Re-inspect
