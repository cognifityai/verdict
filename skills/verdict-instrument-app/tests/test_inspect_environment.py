...
@pytest.mark.parametrize("version", ["0.1.0a8", "0.1.0a9", "0.1.0a10", "0.1.0a11"])
def test_recent_alpha_environment_requires_synchronized_upgrade(
    monkeypatch: pytest.MonkeyPatch,
    version: str,
) -> None:
    report = inspect(monkeypatch, synchronized(version))

    assert report["state"] == "upgrade"
    assert report["action"] == "upgrade"


def test_a12_environment_is_current(monkeypatch: pytest.MonkeyPatch) -> None:
    report = inspect(monkeypatch, synchronized("0.1.0a12"))
...
def test_mixed_versions_fail_closed_to_repair(monkeypatch: pytest.MonkeyPatch) -> None:
    versions = synchronized("0.1.0a12")
...
def test_storage_report_exposes_backend_but_never_secret(
    ...
    report = inspect(monkeypatch, synchronized("0.1.0a12"), storage=storage)
