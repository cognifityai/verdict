...
@pytest.mark.parametrize(
    "version",
    [f"0.1.0a{release}" for release in range(8, 18)],
)
...
def test_a18_environment_is_current(monkeypatch: pytest.MonkeyPatch) -> None:
    report = inspect(monkeypatch, synchronized("0.1.0a18"))
...
def test_mixed_versions_fail_closed_to_repair(monkeypatch: pytest.MonkeyPatch) -> None:
    versions = synchronized("0.1.0a18")
...
def test_storage_report_exposes_backend_but_never_secret(
    ...
    report = inspect(monkeypatch, synchronized("0.1.0a18"), storage=storage)
