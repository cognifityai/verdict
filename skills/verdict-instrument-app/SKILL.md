...
what was executed from what was merely proposed. Verdict `0.1.0a17` is a Python
...
| --- | --- |
| `absent` | Propose one pinned `0.1.0a17` install with only the approved provider, dashboard, semantic, and storage extras. |
| `upgrade` | A synchronized `0.1.0a5` through `0.1.0a16` set exists. Preserve the backend and required extras, back up the store and lockfile, and propose an in-place `0.1.0a17` wheel upgrade. |
...
| `repair` | Missing or mixed Cognifity distributions exist. Show the mismatch and propose one synchronized `0.1.0a17` repair. |
...
[`references/verdict-0.1.0a17.md`](references/verdict-0.1.0a17.md). Its capture
...
For a fresh install or approved upgrade, pin all three distributions to `0.1.0a17`
...
cancellation path. It is not exported as `verdict.shutdown` in `0.1.0a17`.
...
Release `0.1.0a17` does not accept `--yes-spend` or
