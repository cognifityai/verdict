...
Pin the synchronized `0.1.0a14` distributions and verify the installed pipeline
...
- active service/environment configuration separately, while stating that release
  `0.1.0a14` does not persist those two labels on `Trace` rows;
...
Release `0.1.0a14` still constructs the selected embedder when
