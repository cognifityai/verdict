...
Pin the synchronized `0.1.0a15` distributions and verify the installed pipeline
...
- active service/environment configuration separately, while stating that release
  `0.1.0a15` does not persist those two labels on `Trace` rows;
...
Release `0.1.0a15` still constructs the selected embedder when
